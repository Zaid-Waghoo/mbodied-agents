...
@click.option('--model-type', type=click.Choice(['YOLOWorld', 'Grounding DINO'], case_sensitive=False), prompt='Model type', help='The model type to use for detection.')
...
@click.option('--model-src', default="https://api.mbodi.ai/sense/", help='The model source URL.')
@click.option('--image-path', prompt='Image path', help='Path to the image file.')
@click.option('--input-type', type=click.Choice(['bbox', 'coords'], case_sensitive=False), prompt='Input type', help='Type of input data (bbox or coords).')
@click.option('--input-data', prompt='Input data', help='Bounding box coordinates as x1,y1,x2,y2 or pixel coordinates as u,v.')
